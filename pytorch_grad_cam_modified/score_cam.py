...
class ScoreCAM(BaseCAM):
    def __init__(self, model, target_layer, gpu_id=0, reshape_transform=None):
        super(ScoreCAM, self).__init__(model, target_layer, gpu_id, 
    ...
    def get_cam_weights(self,
                        ...
                        grads):
        with torch.no_grad():
            upsample = torch.nn.UpsamplingBilinear2d(size=self.input_tensor.shape[-2 : ])
            ...
            input_tensors = self.input_tensor[:, None, :, :]*upsampled[:, :, None, :, :]
            ...
            for batch_index, img_tensor in enumerate(input_tensors):
                ...
                for i in tqdm.tqdm(range(0, img_tensor.size(0), BATCH_SIZE)):
                    ...
                    outputs, _ = self.model.forward_mean(batch_img, text_tensor)
