...
class AblationCAM(BaseCAM):
    def __init__(self, model, target_layer, gpu_id=0, 
        ...
        super(AblationCAM, self).__init__(model, target_layer, gpu_id, reshape_transform)
    ...
    def get_cam_weights(self,
                        ...
                        grads):
        with torch.no_grad():
            outputs, _ = self.model.forward_mean(self.input_tensor, self.text_tensor)
        ...
        with torch.no_grad():
            ...
            for img_tensor, text_tensor, category in zip(self.input_tensor, self.text_tensor, target_category):
                batch_img_tensor = img_tensor.repeat(BATCH_SIZE, 1, 1, 1)
                batch_text_tensor = text_tensor.repeat(BATCH_SIZE, 1, 1, 1)
                for i in tqdm.tqdm(range(0, number_of_channels, BATCH_SIZE)):
                    ...
                    score, _ = self.model(batch_img_tensor, batch_text_tensor)
                    score = score[:, category].cpu().numpy()
